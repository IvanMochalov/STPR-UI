...
Документ описывает текущую схему взаимодействия кастомных субагентов в проекте и объясняет, почему в некоторых задачах
задействуются не все роли.
...
- `security-auditor` не запускался: задача не затрагивала рискованные зоны (input handling, file/URL processing,
  auth-like logic, dependency risk);
