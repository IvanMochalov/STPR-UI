...
- Only `test-runner` executes repository quality gates (`npm run lint`, `npm run build:lib`, `npm run build:storybook`).
- No other role should rerun these three commands unless `orchestrator` explicitly requests a retry after `debugger`
  fixes.
...
- Prevent duplicate CI-like runs: if latest `test-runner` result is fresh and no files changed since then, do not rerun
  gates.
