...
description:
  Выполняет быстрый и воспроизводимый health-check проекта библиотеки компонентов и Storybook: гигиена репозитория, обязательные quality gates, риски зависимостей и актуальность документации. Использовать, когда пользователь просит project health check, аудит репозитория, проверку готовности к релизу или snapshot рисков сопровождения.
---
...
- Отчет без проверки `build:storybook` считается невалидным.
...
- `ПРОЙДЕНО|ПРОВАЛ` `npm run build:lib`: <краткий итог выполнения>
- `ПРОЙДЕНО|ПРОВАЛ` `npm run build:storybook`: <краткий итог выполнения>
...
- `ПРОВАЛ`: любой Critical риск или провал обязательных gates (`lint`, `build:lib`, `build:storybook`).

## Ограничения

- По возможности держи отчет в пределах 20 строк.
- Не вставляй сырой вывод команд; давай краткую выжимку фактов.
- Не скрывай отсутствие обязательных скриптов: для `lint`, `build:lib`, `build:storybook` это `ПРОВАЛ`.
...
- В отчете есть отдельная строка про `build:storybook` с итогом выполнения.
- В отчете есть отдельные строки про `npm run lint`, `npm run build:lib`, `npm run build:storybook`.
