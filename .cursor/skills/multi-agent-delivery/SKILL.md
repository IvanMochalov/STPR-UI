...
Запускает и контролирует сквозной процесс выполнения задачи: план -> реализация -> документация -> проверки -> финальная валидация.
...
6. `test-runner`
   - запускает `npm run lint`, `npm run build`, `npm run storybook:build`.
7. `debugger`
...
- Критерий готовности: npm run lint, npm run build, npm run storybook:build проходят.
